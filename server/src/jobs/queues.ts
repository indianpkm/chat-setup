...
export const mediaQueue = new Queue<MediaJobData>(QUEUE_NAMES.MEDIA_PROCESSING, {
...
async function processMediaJob(job: Job<MediaJobData>): Promise<{ success: boolean }> {
  const { filename, mimeType, size, uploadedBy } = job.data;

  logger.info(
    { jobId: job.id, filename, mimeType, size, uploadedBy },
...
async function processNotificationJob(
  ...
  logger.info(
    { jobId: job.id, messageId, conversationId, recipientCount: recipientIds.length },
  ...
  logger.info(
    ...
    'Push notification placeholder — integrate FCM/APNs',
  );
...
export async function startWorkers(): Promise<void> {
  mediaWorker = new Worker<MediaJobData>(
  ...
  for (const worker of [mediaWorker, notificationWorker]) {
    worker.on('completed', (job) => {
      logger.info(
        { jobId: job.id, queue: job.queueName, duration: Date.now() - job.timestamp },
        'Job completed',
      );
    });
    ...
    worker.on('failed', (job, err) => {
      logger.error(
        { jobId: job?.id, queue: job?.queueName, err, attemptsMade: job?.attemptsMade },
        'Job failed',
      );
    });
