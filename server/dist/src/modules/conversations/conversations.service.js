...
export async function getOrCreateDM(userId, recipientId) {
...
export async function createGroup(creatorId, input) {
    ...
    if (uniqueIds.length > CONSTANTS.MAX_GROUP_MEMBERS) {
        throw new AppError(`Group cannot exceed ${CONSTANTS.MAX_GROUP_MEMBERS} members`, 400);
    }
...
export async function updateGroup(conversationId, userId, input) {
...
export async function addParticipants(conversationId, requesterId, userIds) {
    const membership = await prisma.conversationParticipant.findUnique({
    ...
    await prisma.$transaction(userIds.map((userId) => prisma.conversationParticipant.upsert({
        where: { conversationId_userId: { conversationId, userId } },
...
export async function removeParticipant(conversationId, requesterId, targetUserId) {
